...
typedef struct Character {
  ...
  Character(char content, const std::string &prefix) : prefix(prefix), content(content) {}
  ...
  bool operator!=(const Character &rhs) const noexcept {
    return rhs.prefix != prefix || ((content == 0 ? ' ' : content) != (rhs.content == 0 ? ' ' : rhs.content));
  }
} Character;
typedef class Screen {
  ...
  bool _test(const Coord &pos) const noexcept { return pos.x < _size.x && pos.y < _size.y; }

public:
  Screen(const Coord &size)
      : current(std::vector<std::vector<Character>>(size.y, std::vector<Character>(size.x))),
        buffer(std::vector<std::vector<Character>>(size.y, std::vector<Character>(size.x))), _size(size), dirty(false) {
    _init();
  }
  void show() {
    if (!dirty)
    ...
    dirty = false;
    std::cout << "\x1b[" << _size.y << ";" << _size.x << "H\x1b[0m" << std::flush;
  }
  void clear() {
    dirty = true;
    for (size_t y = 0; y < _size.y; y++) {
